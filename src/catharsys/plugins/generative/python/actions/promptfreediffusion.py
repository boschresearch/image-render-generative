...
def Run(_xCfg: CConfigList) -> None:
    from .lib.cls_prompt_free_diffusion import CPFD
    ...
    for sModelId, lConfigs in mConfigsForModels.items():
        ...
        sEnv = str(pathCodeBase / "envs" / "Prompt-Free-Diffusion")

        sModelCodePath = str(pathCodeBase / "models" / sModelId)
        sCodePath = str(Path(__file__).parent / f"lib/call_{sModelId}.py")

        if platform.system() == "Linux":
            sCode = f"python {sCodePath}"
            sExpression = (
                # f"conda run -p {sEnv} {sCode} --model_code_path {sModelCodePath} --config_list_path {sConfigListPath}"
                # TODO make proper local env, probably integrate with the generative action, just different Model :P
                # f"conda run -n prompt-free-diffusion {sCode} --model_code_path {sModelCodePath} --config_list_path {sConfigListPath}"
                f"conda run -p {sEnv} {sCode} --model_code_path {sModelCodePath} --config_list_path {sConfigListPath}"        
            )
